...
def main():
    st.title("Customer Satisfaction Pipeline with ZenML and MLflow")

    high_level_image = Image.open("_assets/steps.jpeg")
    st.image(high_level_image, caption="High Level Pipeline")

    whole_pipeline_image = Image.open("_assets/image.png")
    ...
    if st.button("Results"):
        st.write(
            "We have experimented with two ensemble and Deep Learning based models and compared the performance of each model."
        )

        df = pd.DataFrame(
            {
                "Models": ["Xgboost", "Random Forest", "Deep Learning Neural Networks"],
